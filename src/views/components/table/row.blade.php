...
<div class="pt-5 pr-0 pb-0 pl-0 mt-5 mr-0 mb-0 ml-0">
    <div class="sm:flex sm:items-center sm:justify-between sm:space-x-5">
        <div class="flex items-center flex-1 min-w-0">
            ...
            @else
                <img src="{{ $item->preview_image ? $item->preview_image : "https://i.pinimg.com/originals/96/02/08/9602083f42463bb813399310d72233c5.png" }}"
      ...
      sm:mt-0">
            ...
            <x-onix::modal.addEditTemplateModal
                :modalId="'edit-' . $item->id"
                :action="route('onix.templates.edit', $item->id)"
                :componentId="$item->componentId"
                :name="$item->name"
                :slug="$item->slug"
                :css_path="$item->css_file"
                :js_path="$item->js_file"
                :sync="$item->component_sync" :edit="true" :templates="$templates"
            />
            <x-onix::modal.modalDelete :modalId="'delete-' . $item->id" :action="route('onix.templates.delete', $item->id)" />
        @endif
        </div>
    </div>
</div>
